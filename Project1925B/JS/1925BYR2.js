...
function filterBy(year){
    var filtery = ['==', 'Yr', year];
    map.setFilter('Provinces', filtery);
    document.getElementById('active-year').textContent = years[active-year];
}

// Load the map
map.on('load', function() {
d3.json('mapbox://rex13.bqapiacs',
function(err, data){
    if (err) throw err;
...
return d;
    ...
    document.getElementById('slider').addEventListener('input', function(e) {
        var year = parseInt(e.target.value, 1);
        filterBy(year);
        });
    });
}); // do not remove again
